...
Using PnP, pose of the plug holder is also identified.
"""
...
class PlugDetector(Node):
    def __init__(self):
        super().__init__("plug_holder_detector_node")
        ...
        self.boxPose_pub = self.create_publisher(Pose, "/holder_pose", 10)
    ...
    def solveCorrespondence(self, classes, xyxy_arr, xywh_arr):
        ...
        for idx, cls in enumerate(classes):
            if (cls == labels.holder):
                boxArr.append(xyxy_arr[idx])
            elif (cls == labels.bolt):
        ...
        if (len(tempScrewArr) >= 5):
            ...
            for idx, s_xy in enumerate(screw_xy):
                if ((idx in pt1_idx or idx == pt3_idx) or idx == pt4_idx):
                    continue
                pt2_idx = idx
                break
