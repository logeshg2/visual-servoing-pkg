...
"""Script to perform image based visual servoing on extension box"""
...
class IBVS_extension_box(Node):
    def __init__(self):
        super().__init__("ibvs_ext_box_node")
        ...
        self.curScrewDepths = np.array([-1.0, -1.0, -1.0, -1.0])
        ...
        self.matchPoints_sub = self.create_subscription(Int64MultiArray, "/screws_coord", self.matched_points_cb, 10)
        self.boxPose_sub = self.create_subscription(Pose, "/box_pose", self.box_pose_cb, 10)
    ...
    def matched_points_cb(self, msg):
        ...
        if (msg.data is not None and (msg.data[0] != -1)):
            ...
            self.curScrew = np.array(msg.data).reshape((4, 2))
        else:
            self.curScrew = None
    ...
    def computeDesiredInteractionMat(self):
        ...
        for point in self.desiredScrew:
            tempLst.append(
                self.computeInteractionMatrix(point[0], point[1], self.refDepth)
            )
    ...
    def computeCurrentInteractionMat(self):
        ...
        for idx, point in enumerate(self.curScrew):
            tempLst.append(
                self.computeInteractionMatrix(point[0], point[1], self.curScrewDepths[idx])
            )
    ...
    def computeScrewDepth(self):
        ...
        for idx, pt in enumerate(self.objectPoints):
            cTa = self.boxPose                  # camera to box pose
            aTp = np.eye(4)                     # aruco to corner point pose
            aTp[0:3, 3] = pt
            cTp = cTa @ aTp                     # camera to corner point pose
            # store depth (in camera frame)
            self.curScrewDepths[idx] = cTp[2, 3]   # depth (Z value)   
    ...
    def computeCamVel(self):
        ...
        # compute current screws depth depth
        self.computeScrewDepth()
        ...
        tempLst = []
        for refPoint, curPoint in zip(self.desiredScrew, self.curScrew):
            tempLst.extend(
                self.computeImgPointVel(refPoint, curPoint).flatten().tolist()
            )
        self.pixelVel = np.array(tempLst).reshape(((4 * 2), 1))     # column vector
    ...
    def computeEEVel(self):
        ...
        if (self.boxPose is not None and self.curScrew is not None ):
